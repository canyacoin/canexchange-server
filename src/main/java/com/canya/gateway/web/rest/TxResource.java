...
import com.canya.gateway.repository.CMCRepository;
...
import com.canya.gateway.web.websocket.CanYaCoin;
...
public class TxResource {
	...
	private final CMCRepository cMCRepository;
	...
	public TxResource(UserRepository userRepository, UserService userService, MailService mailService,
			...
			TransactionService transactionService, CMCRepository cMCRepository) {
	...
	public void saveTransaction(@Valid @RequestBody TransactionDTO transaction) {
		...
		log.debug("Tx Registered {}", tx.getId());
	...
	public void sentMail(@RequestBody Transaction tx) {
		Transaction transaction = transactionRepository.findOneByKey(tx.getKey());
		Context context = new Context();
		context.setVariable("order", transaction.getKey());
	...
	public ResponseEntity<Transaction> getAllUsers(@PathVariable String id) {
		...
		try {
			Transaction transaction = transactionRepository.findOneByKey(id);
	...
	public Transaction order(@PathVariable String orderid) {
		...
		try {
			Transaction transaction = transactionRepository.findOneByKey(orderid);
			return transaction;
	...
	public void sendStagingActivity(@RequestBody TransactionDTO txData) {
		...
		Transaction txDatac = transactionRepository.findOneByKey(txData.getKey());
